...
//精确查询单条数据
var selectDomainSQL string = "SELECT id,name,tenant_id,company,enable FROM hlr_domain where id=%d"
var selectGroupSQL string = "SELECT id,name,group_desc,parent_id,domain_id FROM hlr_group where id=%d"
var selectUserSQL string = "SELECT id,username,password,group_id FROM hlr_user where id=%d"

//模糊查询
var patternMatchDomainsSQL string = "SELECT id,name,tenant_id,company,enable FROM hlr_domain WHERE name like '%%%s%%' ORDER BY update DESC LIMIT %d OFFSET %d"
var patternMatchGroupsSQL string = "SELECT id,name,group_desc,parent_id,domain_id FROM hlr_group WHERE domain_id=%d ORDER BY name DESC LIMIT %d OFFSET %d"
...
var patternMatchUsersSQL string = `SELECT u.id,u.username,u.password,u.group_id,u.update,g.parent_id as parent_group_id,
	...
	from hlr_user as u 
	LEFT JOIN hlr_group as g on u.group_id=g.id
	LEFT JOIN hlr_group as x on g.parent_id=x.id
	LEFT JOIN hlr_domain as d on g.domain_id=d.id
	%s
	ORDER BY u.update DESC 
	LIMIT %d OFFSET %d`

//group总数
var selectCountGroupSQL string = "SELECT COUNT(*) FROM hlr_group WHERE domain_id=%d"

//domain总数
var selectCountDomainSQL string = "SELECT COUNT(*) FROM hlr_domain  WHERE name like '%%%s%%'"

//组中特定user总数
var patternCountUserByGroupIDSQL string = `SELECT COUNT(*) FROM hlr_user as u 
	LEFT JOIN hlr_group AS g ON u.group_id=g.id 
	LEFT JOIN hlr_group AS x ON g.parent_id=x.id
	WHERE x.id=%d or g.id=%d`

//域中特定user总数
var patternCountUserByDomainIDSQL string = `SELECT COUNT(*) FROM hlr_user AS u
				LEFT JOIN hlr_group AS g ON g.id=u.group_id 
				LEFT JOIN hlr_domain AS d ON g.domain_id=d.id 
				WHERE d.id=%d`
...
//查找domain下的username
var selectUsernameFromDomainSQL string = "SELECT u.username FROM hlr_user as u INNER join hlr_group as g on u.group_id=g.id WHERE g.domain_id=%d and u.username='%s' LIMIT 1"

var selectDomainIDByNameSQL string = "SELECT id FROM hlr_domain WHERE name='%s'"
var selectGroupIDByNameSQL string = "SELECT id FROM hlr_group WHERE name='%s'"
var selectUserIDByNameSQL string = "SELECT id FROM hlr_user WHERE username='%s'"

var selectRealmByIDSQL string = "SELECT name FROM hlr_domain WHERE id=%d"
var selectGroupNameByIDSQL string = "SELECT name FROM hlr_group WHERE id=%d"
var selectUserNameByIDSQL string = "SELECT name FROM hlr_user WHERE id=%d"

var selectOneGroupIDByDomainIDSQL string = "SELECT id FROM hlr_group WHERE domain_id=%d LIMIT 1"
var selectOneGroupIDByParentIDSQL string = "SELECT id FROM hlr_group WHERE parent_id=%d LIMIT 1"
var selectOneUserIDByGroupIDSQL string = "SELECT id FROM hlr_user WHERE group_id=%d LIMIT 1"
